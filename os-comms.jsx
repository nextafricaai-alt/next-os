...
const CF_STORAGE_KEY = 'next_os_client_forms_v1';

function cfLoadForms() {
  try { return JSON.parse(localStorage.getItem(CF_STORAGE_KEY) || '[]'); }
  catch { return []; }
}
function cfSaveForms(forms) {
  localStorage.setItem(CF_STORAGE_KEY, JSON.stringify(forms));
}
...
const FormBuilder = ({ form, onSave, onCancel }) => {
  const [draft, setDraft] = React.useState(() => form || {
    id: cfUid(), title: '', description: '', fields: [], status: 'draft',
...
const ShareModal = ({ form, onClose }) => {
  ...
  // Build a shareable URL — client-form.html with form data encoded in the hash
  const baseUrl = window.location.origin + window.location.pathname.replace(/[^/]*$/, '') + 'client-form.html';
  const encoded = encodeURIComponent(btoa(unescape(encodeURIComponent(JSON.stringify({
    id: form.id, title: form.title, description: form.description,
    fields: form.fields,
    callbackKey: CF_STORAGE_KEY,
  })))));
  const shareUrl = `${baseUrl}#form=${encoded}`;
...
const CommsPage = ({ onNavigate }) => {
  const [forms, setForms] = React.useState(cfLoadForms);
  ...
  /* Persist + listen for new responses via localStorage events */
  React.useEffect(() => {
    const handler = (e) => {
      if (e.key === CF_STORAGE_KEY) {
        const updated = cfLoadForms();
        setForms(updated);
      }
    };
  ...
  /* Poll every 5 seconds when tab is active (handles same-browser submissions) */
  React.useEffect(() => {
    const id = setInterval(() => {
      const fresh = cfLoadForms();
      setForms(prev => {
        const prevTotal = prev.reduce((s, f) => s + (f.responses || []).length, 0);
        const freshTotal = fresh.reduce((s, f) => s + (f.responses || []).length, 0);
        if (freshTotal > prevTotal) {
          const diff = freshTotal - prevTotal;
          setActivity(a => [{
            icon: '📥', ts: new Date().toISOString(),
            text: `${diff} new form response(s) received`,
          }, ...a].slice(0, 20));
          if (window.NEXT_OS?.success) {
            window.NEXT_OS.success('Form Submission', `${diff} new client response(s) arrived.`, { actionUrl: 'os://comms' });
          }
          return fresh;
        }
        return prev;
  ...
  const persist = (updated) => { setForms(updated); cfSaveForms(updated); };

  const handleSave = (draft) => {
    const exists = forms.find(f => f.id === draft.id);
    let updated;
    if (exists) {
      updated = forms.map(f => f.id === draft.id ? { ...draft } : f);
    } else {
      updated = [...forms, { ...draft, status: 'draft', responses: [] }];
  ...
  const handleToggleStatus = (form) => {
    ...
    persist(forms.map(f => f.id === form.id ? { ...f, status: next } : f));
  ...
  const handleViewResponses = (form) => {
    // Mark all responses as seen
    const updated = forms.map(f =>
      f.id === form.id
        ? { ...f, responses: (f.responses || []).map(r => ({ ...r, seen: true })) }
        : f
    );
    persist(updated);
    setViewing(updated.find(f => f.id === form.id));
  };
