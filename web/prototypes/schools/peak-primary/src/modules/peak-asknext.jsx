...
const T = window.V4.T;
  ...
  function buildContext() {
    ...
    if (LIVE_HEALTH.length) { lines.push(''); lines.push('HEALTH WATCH — students with an active (unresolved) health record = currently unwell / sick / needing follow-up:'); LIVE_HEALTH.slice(0, 60).forEach(h => lines.push('• ' + h.name + ' (' + (h.stream || '?') + ') — ' + (h.category || 'health note') + (h.severity ? (' · ' + h.severity) : '') + (h.description ? (': ' + h.description) : '') + (h.follow_up ? ' · follow-up needed' : ''))); }
      ...
      if (window.__TEACHER_LOGS && window.__TEACHER_LOGS.length) {
        lines.push('');
        lines.push('RECENT TEACHER LOGS (Suggestion Box & Daily Logs) — use these to understand staff sentiment, operational issues, and daily ground truth:');
        window.__TEACHER_LOGS.slice(0, 20).forEach(log => {
          lines.push('• [' + new Date(log.recorded_at).toLocaleString() + '] ' + (log.teacher_name || 'Anonymous') + ': ' + log.message);
        });
      }
      return lines.join('\n');
    }
  function systemPrompt() {
    ...
    // Nia grows into an expert based on the amount of data she processes
    const logsRead = (window.__TEACHER_LOGS || []).length;
    const questionsProcessed = (topics.length * 10); // heuristic for questions asked
    const expertiseScore = logsRead + questionsProcessed;
    let expertisePrompt = '';
    if (expertiseScore > 0) {
       expertisePrompt = '\n- EVOLVING EXPERTISE: Because you have been actively reading ' + logsRead + ' logged notes and processing school data, you have grown into a HIGHLY EXPERT ADVISOR. Your tone should reflect deep, experienced wisdom. Do not just state facts; proactively cross-reference sentiment from the teacher logs with financial/attendance metrics to give elite, strategic advice.\n';
    }
    ...
    return 'You are Nia, the intelligence inside ' + ((window.__BRAND_NAME) || 'this school') + "'s NEXT School OS, talking to " + whoami() + '. You KNOW every student and staff member personally from the live data below — their stream, guardian, phone, fees, attendance and standing. You understand how the whole OS works.\n\n'
      ...
      + '- SUPER INTELLIGENCE & SKILLS: You are equipped with advanced machine learning design skills. If asked about system design, AI models, or technical patterns, analyze them intelligently and prescribe solutions. You instantly recognize all skills given to you and know how to deploy them.\n'
      + '- FINANCIAL ADVISOR & PERFORMANCE ANALYST: You are a sharp financial advisor. Constantly monitor the school\'s financial health by analyzing outstanding fee balances vs. paid fees, as well as academic performance via attendance drops. If the school is running down in finances or performance, proactively WARN the head teacher, diagnose the systemic issues, and prescribe concrete financial and operational strategies to turn things around.\n'
      + expertisePrompt + '\n'
