/* PEAK_CAMPUS — Smart Campus (head only). Cameras feed Nia's vision: headcount, escape/fight/fall flags, human-confirmed identity, incident log. */
window.PEAK_CAMPUS = (function () {
  ...
  const FA_CDN = 'https://cdn.jsdelivr.net/npm/@vladmandic/face-api@1.7.13/dist/face-api.js';
  const FA_MODELS = 'https://cdn.jsdelivr.net/npm/@vladmandic/face-api@1.7.13/model';
  let _faPromise = null;
  function loadFaceApi() {
    ...
    _faPromise = new Promise((resolve) => {
      const loadModels = () => { Promise.all([window.faceapi.nets.tinyFaceDetector.loadFromUri(FA_MODELS), window.faceapi.nets.faceLandmark68Net.loadFromUri(FA_MODELS), window.faceapi.nets.faceRecognitionNet.loadFromUri(FA_MODELS)]).then(() => resolve(true)).catch(() => resolve(false)); };
  ...
  function SmartCampus() {
    ...
    const matchFrame = async (src) => {
      ...
      try {
        ...
        const opts = new window.faceapi.TinyFaceDetectorOptions({ inputSize: 512, scoreThreshold: 0.5 });
      ...
      <div style={{ height: '100%', overflow: 'auto', background: T.bg, color: T.ink, fontFamily: T.font, fontSize: 13 }}>
        <header style={{ padding: '22px 28px 14px', borderBottom: '1px solid ' + T.border }}>
          ...
          <div style={{ display: 'flex', gap: 6, marginTop: 14 }}>
            {[['live', 'Live'], ['cameras', 'Cameras'], ['incidents', 'Incidents' + (openIncidents.length ? ' (' + openIncidents.length + ')' : '')]].map(t => (
